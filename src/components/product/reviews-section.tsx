...
function StarsRow({ value }: { value: number }) {
  return (
    <span className="text-sm tracking-wide text-cta" aria-label={`${value} de 5 estrellas`}>
      {"★".repeat(value)}
      <span className="text-foreground-disabled">{"★".repeat(5 - value)}</span>
    </span>
  );
}
  ...
  return (
    <section className="mt-20" id="resenas">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div>
          {reviews.length === 0 ? (
            <p className="border border-dashed border-border px-5 py-10 text-center text-sm text-foreground-disabled">
        ...
        <form onSubmit={submit} className="h-fit border border-border bg-background-secondary/50 p-5">
          ...
          <p className="mt-2 text-center text-[11px] leading-relaxed text-foreground-disabled">
